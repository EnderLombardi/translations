sap.ui.controller("airbus.mes.settings.FilterPlantData",{
      ...
      */
      ...
      onInit : function() {
    	  ...
    	  oRouter.getRoute("settings").attachMatched(this._onRouteMatched, this);
    	  
    	  
    	  // Set i18n Model
    	  var i18nModel = new sap.ui.model.resource.ResourceModel({
      		bundleUrl : "settings/i18n/i18n.properties",
      		bundleLocale : sap.ui.getCore().getConfiguration().getLanguage()
    	  });
    	  this.getView().setModel(i18nModel, "i18n");
    	  
    	  //Include CSS file
    	  jQuery.sap.includeStyleSheet("settings/css/SettingScreen.css");
      ...
      _onRouteMatched : function (oEvent) {
		...
		if (this._oRouterArgs.query) {
			this.routerNav = this._oRouterArgs.query.nav;
			
			//Set Navigate Button Text
			switch(this.routerNav){
				case "worktracker":
					this.getView().byId("navBtn").setText("Go to Work Tracker");
					break;
				case "stationtracker":
					this.getView().byId("navBtn").setText("Go to Station Tracker");
					break;
				default:
					this.getView().byId("navBtn").setText("Go back");
					this.routerNav = "back";
			}
...
//                                             }
      ...
      navigate : function(oEvent){
          ...
          if(airbus.mes != undefined){
        	  sap.ui.getCore().byId("mes--settingsButton").setEnabled(true);
          };
  		  
          if (this.routerNav == "back" ) {
         	  window.history.go(-1);
          }
          else
        	  this.getOwnerComponent().getRouter().navTo(this.routerNav);
