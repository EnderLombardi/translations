...
var viewers=this.options.type.replace(" ","").split(",");if(viewers.length<1)viewers[0]="html5";me._flashVer="10.2";me._flashFABridgeId="ReaderControl";me._silverlightVer="4.0";me._createViewer(viewers)},_createViewer:function(viewers){var me=this;me.selectedType="none";if(this.isMobileDevice()){viewers=Array("html5Mobile");me.selectedType="html5Mobile";if(this.options.mobileRedirect){var newLocation=this.options.html5MobilePath+this._getHTML5OptionsURL();window.location=newLocation;return}}var allowHTML5=
false;var allowSilverlight=false;var allowFlash=false;for(var i=0;i<viewers.length;i++){if(viewers[i].toLowerCase()==="html5mobile"){allowHTML5=true;if(me._testHTML5()){if(this.options.mobileRedirect){me.selectedType="html5Mobile";var newLocation=this.options.html5MobilePath+this._getHTML5OptionsURL();window.location=newLocation;return}if(me.isSameOrigin(decodeURIComponent(me.options.initialDoc))||me._testCORS()){me.selectedType="html5Mobile";break}}}if(viewers[i].toLowerCase()==="html5"){allowHTML5=
true;if(me._testHTML5()){var sameOrigin=me.isSameOrigin(decodeURIComponent(me.options.initialDoc));if(sameOrigin||(me._testCORS()||me._testSilverlight(me._silverlightVer))){me.selectedType="html5";break}}}else if(viewers[i].toLowerCase()==="silverlight"){allowSilverlight=true;if(me._testSilverlight(me._silverlightVer)){me.selectedType="silverlight";break}}else if(viewers[i].toLowerCase()==="flash"){allowFlash=true;if(me._testFlash(me._flashVer)){me.selectedType="flash";break}}}if(me.selectedType===
"html5")me._createHTML5();else if(me.selectedType==="html5Mobile")me._createHTML5Mobile();else if(me.selectedType==="silverlight")me._createSilverlight();else if(me.selectedType==="flash")me._createFlash();else{var supportErrorArray=new Array;supportErrorArray.push("Please");if(allowHTML5){supportErrorArray.push("use an HTML5 compatible browser");if(allowSilverlight||allowFlash)supportErrorArray.push("or")}if(allowSilverlight||allowFlash){var runtimesStr="("+(allowSilverlight?"Silverlight":"")+(allowSilverlight&&
allowFlash?" or ":"")+(allowFlash?"Flash":"")+")";supportErrorArray.push("install the necessary runtime "+runtimesStr)}$(me.element).append('<div id="webviewer-browser-unsupported">'+supportErrorArray.join(" ")+".</div>")}},_viewerLoaded:function(iframe){var me=this;me._trigger("ready");var viewerWindow=iframe.contentWindow;if(typeof me.options.encryption!=="undefined"){var decrypt=viewerWindow.CoreControls.Encryption.Decrypt;var doc=decodeURIComponent(me.options.initialDoc);var streaming=me.options.streaming;
...
function(event,data1,data2){me._trigger(event.type,null,data1,data2)})},_getHTML5OptionsURL:function(){if(this.selectedType==="html5"){if(this.options.html5Options)$.extend(this.options,this.options.html5Options)}else if(this.selectedType==="html5Mobile")if(this.options.html5MobileOptions)$.extend(this.options,this.options.html5MobileOptions);var options=this.options;var url="";if(typeof options.initialDoc!=="undefined")url+="#d="+options.initialDoc;if(options.streaming)url+="&streaming=true";if(options.encryption)url+=
...
"&readonly=1";if(typeof options.annotationUser!=="undefined")url+="&user="+options.annotationUser;if(typeof options.annotationAdmin!=="undefined")url+="&admin="+(options.annotationAdmin?1:0);if(typeof options.custom!=="undefined")url+="&custom="+encodeURIComponent(options.custom);if(typeof options.showToolbarControl!=="undefined")url+="&toolbar="+(options.showToolbarControl?"true":"false");if(typeof options.xdomainProxyUrl!=="undefined"){var urls;if(typeof options.xdomainProxyUrl==="string")urls=
{url:options.xdomainProxyUrl};else urls=options.xdomainProxyUrl;url+="&xdomain_urls="+encodeURIComponent(JSON.stringify(urls))}if(url.length>0&&url[0]==="&")url="#"+url.slice(1);return url},_createHTML5:function(){var me=this;var iframeSource=this.options.html5Path+this._getHTML5OptionsURL();if(!me.isSameOrigin(decodeURIComponent(me.options.initialDoc))&&(!me._testCORS()&&this._testSilverlight(me._silverlightVer)))iframeSource+="&useSilverlightRequests=true";var $rcFrame=$(document.createElement("iframe"));
$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0,width:"100%",height:"100%",allowFullScreen:true,webkitallowfullscreen:true,mozallowfullscreen:true});var outerWindow=window;$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;outerWindow.CoreControls=outerWindow.CoreControls||{};outerWindow.CoreControls.DisplayModes=this.contentWindow.CoreControls.DisplayModes;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",
...
var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",function(event){me.instance=iframe.contentWindow.readerControl;me._viewerLoaded(iframe)});else me._viewerLoaded(iframe)});$(this.element).append($rcFrame);return $rcFrame},_getSilverlightInitParam:function(){var options=this.options;var initParam="";initParam+="ShowToolbarControl="+(options.showToolbarControl?"true":"false");if(typeof options.initialDoc!=="undefined"){initParam+=",DocumentUri="+
function decode(str){return unescape(str.replace(/\+/g," "))}(options.initialDoc);if(options.streaming)initParam+=",Streaming=true";else initParam+=",Streaming=false";if(options.enableAnnotations)initParam+=",a=true";else initParam+=",a=false";if(options.serverUrl){var serverUrl=this._correctRelativePath(options.serverUrl);initParam+=",server_url="+serverUrl}if(options.documentId)initParam+=",did="+options.documentId;if(options.config)initParam+=",config="+options.config;if(options.enableOfflineMode)initParam+=
",offline=true";if(options.enableReadOnlyMode)initParam+=",readonly=true";if(options.annotationUser)initParam+=",user="+options.annotationUser;if(typeof options.annotationAdmin!=="undefined")if(options.annotationAdmin)initParam+=",admin=true";else initParam+=",admin=false";if(typeof options.custom!=="undefined"&&options.custom!==null)initParam+=",custom="+encodeURIComponent(options.custom);if(typeof options.nogui)initParam+=",nogui="+options.nogui}return initParam},_createSilverlight:function(){var me=
this;if(this.options.silverlightOptions)$.extend(this.options,this.options.silverlightOptions);var options=this.options;var initParam=this._getSilverlightInitParam();var objectParams={"source":options.silverlightPath,"minRuntimeVersion":"4.0.50401.0","autoUpgrade":"true"};$.extend(objectParams,options.silverlightObjectParams);var objectElement=Silverlight.createObject(options.silverlightPath,null,me.rcId,objectParams,{onLoad:function(sender,args){me.instance=sender.Content.ReaderControl;me.instance.EnableAnnotations=
options.enableAnnotations;me.instance.onPropertyChanged=function(sender2,args2){switch(args2.name){case "CurrentPageNumber":me._trigger("pageChanged",null,{pageNumber:args2.pageNumber});break;case "DisplayMode":me._trigger("displayModeChanged");break;case "LayoutMode":me._trigger("layoutModeChanged");break;case "Zoom":me._trigger("zoomChanged");break;case "FitMode":me._trigger("fitModeChanged");break;case "ToolMode":me._trigger("toolModeChanged");break}};me._trigger("ready");me.instance.onDocumentLoaded=
function(sender3,args3){me._trigger("documentLoaded")}},onError:function(){console.error("Silverlight onError")}},initParam,null);$(objectElement).attr({"width":"100%","height":"100%"}).appendTo(me.element)},_createFlash:function(){var me=this;if(this.options.flashOptions)$.extend(this.options,this.options.flashOptions);var options=this.options;var swfUrlStr=options.flashPath;var flashObj=document.createElement("object");flashObj.setAttribute("id",me.rcId);$(flashObj).appendTo(me.element);var xiSwfUrl=
"";var flashvars={};flashvars.bridgeName=me._flashFABridgeId;if(options.initialDoc!==undefined)flashvars.d=options.initialDoc;if(options.streaming!==undefined)flashvars.streaming=options.streaming;flashvars.showToolbarControl=options.showToolbarControl;if(options.flashSockets===true)flashvars.NonStreamingMode="SOCKETS";else flashvars.NonStreamingMode="AJAX";var params={};params.allowfullscreen="true";params.allowFullScreenInteractive="true";if(!me.isSameOrigin(decodeURIComponent(me.options.initialDoc))&&
(!me._testCORS()&&flashvars.NonStreamingMode==="AJAX"))flashvars.streaming=true;var attributes={};attributes.id=me._flashFABridgeId;attributes.name=me._flashFABridgeId;if(typeof options.encryption!=="undefined")createHttpAjaxPartRetriever(me._flashFABridgeId,decodeURIComponent(flashvars.d),window.CoreControls.Encryption.DecryptSynchronous,options.encryption);swfobject.embedSWF(swfUrlStr,me.rcId,"100%","100%",me._flashVer,xiSwfUrl,flashvars,params,attributes,function(e){if(e.success)FABridge.addInitializationCallback(me._flashFABridgeId,
...
"html5"||this.selectedType==="html5Mobile")return this.getInstance().getToolMode();else if(this.selectedType==="silverlight"||this.selectedType==="flash")return this.getInstance().GetToolMode()},setToolMode:function(toolMode){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().setToolMode(toolMode);else if(this.selectedType==="silverlight"||this.selectedType==="flash")this.getInstance().SetToolMode(toolMode)},fitWidth:function(){if(this.selectedType==="html5"||this.selectedType===
"html5Mobile")this.getInstance().fitWidth();else if(this.selectedType==="flash"||this.selectedType==="silverlight")this.getInstance().FitWidth()},fitHeight:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().fitHeight();else if(this.selectedType==="flash")console.warn("Unsupported method fitHeight.");else if(this.selectedType==="silverlight")this.getInstance().FitHeight()},fitPage:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().fitPage();
else if(this.selectedType==="flash"||this.selectedType==="silverlight")this.getInstance().FitPage()},getFitMode:function(){if(this.selectedType==="html5"){var fitMode=this.getInstance().getFitMode();var FitModeEnums=this.getInstance().docViewer.FitMode;switch(fitMode){case FitModeEnums.FitWidth:return PDFTron.WebViewer.FitMode.FitWidth;case FitModeEnums.FitHeight:return PDFTron.WebViewer.FitMode.FitHeight;case FitModeEnums.FitPage:return PDFTron.WebViewer.FitMode.FitPage;case FitModeEnums.Zoom:return PDFTron.WebViewer.FitMode.Zoom;
default:console.warn("Unsupported method getFitMode")}}else if(this.selectedType==="silverlight")return this.getInstance().getFitMode();else if(this.selectedType==="flash"){var camel=this.getInstance().GetFitMode();return camel.charAt(0).toUpperCase()+camel.slice(1)}else console.warn("Unsupported method getFitMode")},setFitMode:function(fitMode){if(fitMode===PDFTron.WebViewer.FitMode.FitWidth)this.fitWidth();else if(fitMode===PDFTron.WebViewer.FitMode.FitHeight)if(this.selectedType==="flash")this.fitPage();
else this.fitHeight();else if(fitMode===PDFTron.WebViewer.FitMode.FitPage)this.fitPage();else if(fitMode===PDFTron.WebViewer.FitMode.Zoom)this.zoom();else console.warn("Unsupported fit mode: "+fitMode)},zoom:function(){if(this.selectedType==="html5"||this.selectedType==="html5Mobile")this.getInstance().fitZoom();else if(this.selectedType==="flash")this.getInstance().FitZoom();else if(this.selectedType==="silverlight")this.getInstance().Zoom()},goToFirstPage:function(){if(this.selectedType==="html5"||
...
"html5Mobile")this.getInstance().goToPrevPage();else if(this.selectedType==="flash")this.getInstance().GoToPrevPage();else if(this.selectedType==="silverlight"){var currentPage=this.getInstance().CurrentPageNumber;if(currentPage<=1)return;currentPage=currentPage-1;this.getInstance().CurrentPageNumber=currentPage}},loadDocument:function(url,documentID){if(this.selectedType==="html5"||this.selectedType==="html5Mobile"){if(documentID)this.getInstance().doc_id=documentID;this.getInstance().loadDocument(this._correctRelativePath(url),
this.options.streaming)}else this.getInstance().LoadDocument(this._correctRelativePath(url),this.options.streaming)},searchText:function(pattern,searchModes){var mode=0;var modes=searchModes;if(typeof modes==="string")modes=searchModes.split(",");if(typeof modes!=="undefined")for(var i=0;i<modes.length;i++)if(modes[i]==="CaseSensitive")mode+=1;else if(modes[i]==="WholeWord")mode+=2;else if(modes[i]==="SearchUp")mode+=4;else if(modes[i]==="PageStop")mode+=8;else if(modes[i]==="ProvideQuads")mode+=
16;else if(modes[i]==="AmbientString")mode+=32;if(this.selectedType==="html5"||this.selectedType==="html5Mobile")if(typeof searchModes==="undefined")this.getInstance().searchText(pattern);else this.getInstance().searchText(pattern,mode);else if(this.selectedType==="silverlight"){var modeString=$.isArray(searchModes)?searchModes.join(","):searchModes;if(typeof modeString==="undefined")modeString="";this.getInstance().SearchText(pattern,modeString)}else if(this.selectedType==="flash")if(typeof searchModes===
...
false,dynamicStylesheet,dynamicStylesheetMedia,autoHideShow=true,ua=function(){var ieVersion=function(){var ua=navigator.userAgent.toLowerCase();var match=/(msie) ([\w.]+)/.exec(ua)||/(trident)(?:.*? rv:([\w.]+)|)/.exec(ua);return match?parseInt(match[2],10):match}();var w3cdom=typeof doc.getElementById!=UNDEF&&(typeof doc.getElementsByTagName!=UNDEF&&typeof doc.createElement!=UNDEF),u=nav.userAgent.toLowerCase(),p=nav.platform.toLowerCase(),windows=p?/win/.test(p):/win/.test(u),mac=p?/mac/.test(p):
/mac/.test(u),webkit=/webkit/.test(u)?parseFloat(u.replace(/^.*webkit\/(\d+(\.\d+)?).*$/,"$1")):false,ie=!+"\v1",playerVersion=[0,0,0],d=null;if(typeof nav.plugins!=UNDEF&&typeof nav.plugins[SHOCKWAVE_FLASH]==OBJECT){d=nav.plugins[SHOCKWAVE_FLASH].description;if(d&&!(typeof nav.mimeTypes!=UNDEF&&(nav.mimeTypes[FLASH_MIME_TYPE]&&!nav.mimeTypes[FLASH_MIME_TYPE].enabledPlugin))){plugin=true;ie=false;d=d.replace(/^.*\s+(\S+\s+\S+$)/,"$1");playerVersion[0]=parseInt(d.replace(/^(.*)\..*$/,"$1"),10);playerVersion[1]=
parseInt(d.replace(/^.*\.(.*)\s.*$/,"$1"),10);playerVersion[2]=/[a-zA-Z]/.test(d)?parseInt(d.replace(/^.*[a-zA-Z]+(.*)$/,"$1"),10):0}}else if(typeof win.ActiveXObject!=UNDEF)try{var a=new ActiveXObject(SHOCKWAVE_FLASH_AX);if(a){d=a.GetVariable("$version");if(d){ie=true;d=d.split(" ")[1].split(",");playerVersion=[parseInt(d[0],10),parseInt(d[1],10),parseInt(d[2],10)]}}}catch(e){}return{w3:w3cdom,pv:playerVersion,wk:webkit,ie:ie,win:windows,mac:mac,ieVersion:ieVersion}}(),onDomLoad=function(){if(!ua.w3)return;
if(typeof doc.readyState!=UNDEF&&doc.readyState=="complete"||typeof doc.readyState==UNDEF&&(doc.getElementsByTagName("body")[0]||doc.body))callDomLoadFunctions();if(!isDomLoaded){if(typeof doc.addEventListener!=UNDEF)doc.addEventListener("DOMContentLoaded",callDomLoadFunctions,false);if(ua.ie&&ua.win){doc.attachEvent(ON_READY_STATE_CHANGE,function(){if(doc.readyState=="complete"){doc.detachEvent(ON_READY_STATE_CHANGE,arguments.callee);callDomLoadFunctions()}});if(win==top)(function(){if(isDomLoaded)return;
try{doc.documentElement.doScroll("left")}catch(e){setTimeout(arguments.callee,0);return}callDomLoadFunctions()})()}if(ua.wk)(function(){if(isDomLoaded)return;if(!/loaded|complete/.test(doc.readyState)){setTimeout(arguments.callee,0);return}callDomLoadFunctions()})();addLoadEvent(callDomLoadFunctions)}}();function callDomLoadFunctions(){if(isDomLoaded)return;try{var t=doc.getElementsByTagName("body")[0].appendChild(createElement("span"));t.parentNode.removeChild(t)}catch(e){return}isDomLoaded=true;
var dl=domLoadFnArr.length;for(var i=0;i<dl;i++)domLoadFnArr[i]()}function addDomLoadEvent(fn){if(isDomLoaded)fn();else domLoadFnArr[domLoadFnArr.length]=fn}function addLoadEvent(fn){if(typeof win.addEventListener!=UNDEF)win.addEventListener("load",fn,false);else if(typeof doc.addEventListener!=UNDEF)doc.addEventListener("load",fn,false);else if(typeof win.attachEvent!=UNDEF)addListener(win,"onload",fn);else if(typeof win.onload=="function"){var fnOld=win.onload;win.onload=function(){fnOld();fn()}}else win.onload=
fn}function main(){if(plugin)testPlayerVersion();else matchVersions()}function testPlayerVersion(){var b=doc.getElementsByTagName("body")[0];var o=createElement(OBJECT);o.setAttribute("type",FLASH_MIME_TYPE);var t=b.appendChild(o);if(t){var counter=0;(function(){if(typeof t.GetVariable!=UNDEF){var d=t.GetVariable("$version");if(d){d=d.split(" ")[1].split(",");ua.pv=[parseInt(d[0],10),parseInt(d[1],10),parseInt(d[2],10)]}}else if(counter<10){counter++;setTimeout(arguments.callee,10);return}b.removeChild(o);
t=null;matchVersions()})()}else matchVersions()}function matchVersions(){var rl=regObjArr.length;if(rl>0)for(var i=0;i<rl;i++){var id=regObjArr[i].id;var cb=regObjArr[i].callbackFn;var cbObj={success:false,id:id};if(ua.pv[0]>0){var obj=getElementById(id);if(obj)if(hasPlayerVersion(regObjArr[i].swfVersion)&&!(ua.wk&&ua.wk<312)){setVisibility(id,true);if(cb){cbObj.success=true;cbObj.ref=getObjectById(id);cb(cbObj)}}else if(regObjArr[i].expressInstall&&canExpressInstall()){var att={};att.data=regObjArr[i].expressInstall;
...
true);if(cb){var o=getObjectById(id);if(o&&typeof o.SetVariable!=UNDEF){cbObj.success=true;cbObj.ref=o}cb(cbObj)}}}}function getObjectById(objectIdStr){var r=null;var o=getElementById(objectIdStr);if(o&&o.nodeName=="OBJECT")if(typeof o.SetVariable!=UNDEF)r=o;else{var n=o.getElementsByTagName(OBJECT)[0];if(n)r=n}return r}function canExpressInstall(){return!isExpressInstallActive&&(hasPlayerVersion("6.0.65")&&((ua.win||ua.mac)&&!(ua.wk&&ua.wk<312)))}function showExpressInstall(att,par,replaceElemIdStr,
callbackFn){isExpressInstallActive=true;storedCallbackFn=callbackFn||null;storedCallbackObj={success:false,id:replaceElemIdStr};var obj=getElementById(replaceElemIdStr);if(obj){if(obj.nodeName=="OBJECT"){storedAltContent=abstractAltContent(obj);storedAltContentId=null}else{storedAltContent=obj;storedAltContentId=replaceElemIdStr}att.id=EXPRESS_INSTALL_ID;if(typeof att.width==UNDEF||!/%$/.test(att.width)&&parseInt(att.width,10)<310)att.width="310";if(typeof att.height==UNDEF||!/%$/.test(att.height)&&
parseInt(att.height,10)<137)att.height="137";doc.title=doc.title.slice(0,47)+" - Flash Player Installation";var pt=ua.ie&&ua.win?"ActiveX":"PlugIn",fv="MMredirectURL="+encodeURI(window.location).toString().replace(/&/g,"%26")+"&MMplayerType="+pt+"&MMdoctitle="+doc.title;if(typeof par.flashvars!=UNDEF)par.flashvars+="&"+fv;else par.flashvars=fv;if(ua.ie&&(ua.win&&obj.readyState!=4)){var newObj=createElement("div");replaceElemIdStr+="SWFObjectNew";newObj.setAttribute("id",replaceElemIdStr);obj.parentNode.insertBefore(newObj,
obj);obj.style.display="none";(function(){if(obj.readyState==4)obj.parentNode.removeChild(obj);else setTimeout(arguments.callee,10)})()}createSWF(att,par,replaceElemIdStr)}}function displayAltContent(obj){if(ua.ie&&(ua.win&&obj.readyState!=4)){var el=createElement("div");obj.parentNode.insertBefore(el,obj);el.parentNode.replaceChild(abstractAltContent(obj),el);obj.style.display="none";(function(){if(obj.readyState==4)obj.parentNode.removeChild(obj);else setTimeout(arguments.callee,10)})()}else obj.parentNode.replaceChild(abstractAltContent(obj),
obj)}function abstractAltContent(obj){var ac=createElement("div");if(ua.win&&ua.ie)ac.innerHTML=obj.innerHTML;else{var nestedObj=obj.getElementsByTagName(OBJECT)[0];if(nestedObj){var c=nestedObj.childNodes;if(c){var cl=c.length;for(var i=0;i<cl;i++)if(!(c[i].nodeType==1&&c[i].nodeName=="PARAM")&&!(c[i].nodeType==8))ac.appendChild(c[i].cloneNode(true))}}}return ac}function createSWF(attObj,parObj,id){var r,el=getElementById(id);if(ua.wk&&ua.wk<312)return r;if(el){if(typeof attObj.id==UNDEF)attObj.id=
id;if(ua.ie&&ua.win){var att="";for(var i in attObj)if(attObj[i]!=Object.prototype[i])if(i.toLowerCase()=="data")parObj.movie=attObj[i];else if(i.toLowerCase()=="styleclass")att+=' class="'+attObj[i]+'"';else if(i.toLowerCase()!="classid")att+=" "+i+'="'+attObj[i]+'"';var par="";for(var j in parObj)if(parObj[j]!=Object.prototype[j])par+='<param name="'+j+'" value="'+parObj[j]+'" />';el.outerHTML='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"'+att+">"+par+"</object>";objIdArr[objIdArr.length]=
attObj.id;r=getElementById(attObj.id)}else{var o=createElement(OBJECT);o.setAttribute("type",FLASH_MIME_TYPE);for(var m in attObj)if(attObj[m]!=Object.prototype[m])if(m.toLowerCase()=="styleclass")o.setAttribute("class",attObj[m]);else if(m.toLowerCase()!="classid")o.setAttribute(m,attObj[m]);for(var n in parObj)if(parObj[n]!=Object.prototype[n]&&n.toLowerCase()!="movie")createObjParam(o,n,parObj[n]);el.parentNode.replaceChild(o,el);r=o}}return r}function createObjParam(el,pName,pValue){var p=createElement("param");
p.setAttribute("name",pName);p.setAttribute("value",pValue);el.appendChild(p)}function removeSWF(id){var obj=getElementById(id);if(obj&&obj.nodeName=="OBJECT")if(ua.ie&&ua.win){obj.style.display="none";(function(){if(obj.readyState==4)removeObjectInIE(id);else setTimeout(arguments.callee,10)})()}else obj.parentNode.removeChild(obj)}function removeObjectInIE(id){var obj=getElementById(id);if(obj){for(var i in obj)if(typeof obj[i]=="function")obj[i]=null;obj.parentNode.removeChild(obj)}}function getElementById(id){var el=
null;try{el=doc.getElementById(id)}catch(e){}return el}function createElement(el){return doc.createElement(el)}function addListener(target,eventType,fn){target.attachEvent(eventType,fn);listenersArr[listenersArr.length]=[target,eventType,fn]}function hasPlayerVersion(rv){var pv=ua.pv,v=rv.split(".");v[0]=parseInt(v[0],10);v[1]=parseInt(v[1],10)||0;v[2]=parseInt(v[2],10)||0;return pv[0]>v[0]||(pv[0]==v[0]&&pv[1]>v[1]||pv[0]==v[0]&&(pv[1]==v[1]&&pv[2]>=v[2]))?true:false}function createCSS(sel,decl,
media,newStyle){if(ua.ie&&ua.mac)return;var h=doc.getElementsByTagName("head")[0];if(!h)return;var m=media&&typeof media=="string"?media:"screen";if(newStyle){dynamicStylesheet=null;dynamicStylesheetMedia=null}if(!dynamicStylesheet||dynamicStylesheetMedia!=m){var s=createElement("style");s.setAttribute("type","text/css");s.setAttribute("media",m);dynamicStylesheet=h.appendChild(s);if(ua.ie&&(ua.win&&(typeof doc.styleSheets!=UNDEF&&doc.styleSheets.length>0)))dynamicStylesheet=doc.styleSheets[doc.styleSheets.length-
1];dynamicStylesheetMedia=m}if(ua.ie&&ua.win){if(dynamicStylesheet&&typeof dynamicStylesheet.addRule==OBJECT)dynamicStylesheet.addRule(sel,decl)}else if(dynamicStylesheet&&typeof doc.createTextNode!=UNDEF)dynamicStylesheet.appendChild(doc.createTextNode(sel+" {"+decl+"}"))}function setVisibility(id,isVisible){if(!autoHideShow)return;var v=isVisible?"visible":"hidden";if(isDomLoaded&&getElementById(id))getElementById(id).style.visibility=v;else createCSS("#"+id,"visibility:"+v)}function urlEncodeIfNecessary(s){var regex=
/[\\\"<>\.;]/;var hasBadChars=regex.exec(s)!=null;return hasBadChars&&typeof encodeURIComponent!=UNDEF?encodeURIComponent(s):s}var cleanup=function(){if(ua.ie&&ua.win)window.attachEvent("onunload",function(){var ll=listenersArr.length;for(var i=0;i<ll;i++)listenersArr[i][0].detachEvent(listenersArr[i][1],listenersArr[i][2]);var il=objIdArr.length;for(var j=0;j<il;j++)removeSWF(objIdArr[j]);for(var k in ua)ua[k]=null;ua=null;for(var l in swfobject)swfobject[l]=null;swfobject=null})}();return{registerObject:function(objectIdStr,
swfVersionStr,xiSwfUrlStr,callbackFn){if(ua.w3&&(objectIdStr&&swfVersionStr)){var regObj={};regObj.id=objectIdStr;regObj.swfVersion=swfVersionStr;regObj.expressInstall=xiSwfUrlStr;regObj.callbackFn=callbackFn;regObjArr[regObjArr.length]=regObj;setVisibility(objectIdStr,false)}else if(callbackFn)callbackFn({success:false,id:objectIdStr})},getObjectById:function(objectIdStr){if(ua.w3)return getObjectById(objectIdStr)},embedSWF:function(swfUrlStr,replaceElemIdStr,widthStr,heightStr,swfVersionStr,xiSwfUrlStr,
flashvarsObj,parObj,attObj,callbackFn){var callbackObj={success:false,id:replaceElemIdStr};if(ua.w3&&(!(ua.wk&&ua.wk<312)&&(swfUrlStr&&(replaceElemIdStr&&(widthStr&&(heightStr&&swfVersionStr)))))){setVisibility(replaceElemIdStr,false);addDomLoadEvent(function(){widthStr+="";heightStr+="";var att={};if(attObj&&typeof attObj===OBJECT)for(var i in attObj)att[i]=attObj[i];att.data=swfUrlStr;att.width=widthStr;att.height=heightStr;var par={};if(parObj&&typeof parObj===OBJECT)for(var j in parObj)par[j]=
parObj[j];if(flashvarsObj&&typeof flashvarsObj===OBJECT)for(var k in flashvarsObj)if(typeof par.flashvars!=UNDEF)par.flashvars+="&"+k+"="+flashvarsObj[k];else par.flashvars=k+"="+flashvarsObj[k];if(hasPlayerVersion(swfVersionStr)){var obj=createSWF(att,par,replaceElemIdStr);if(att.id==replaceElemIdStr)setVisibility(replaceElemIdStr,true);callbackObj.success=true;callbackObj.ref=obj}else if(xiSwfUrlStr&&canExpressInstall()){att.data=xiSwfUrlStr;showExpressInstall(att,par,replaceElemIdStr,callbackFn);
return}else setVisibility(replaceElemIdStr,true);if(callbackFn)callbackFn(callbackObj)})}else if(callbackFn)callbackFn(callbackObj)},switchOffAutoHideShow:function(){autoHideShow=false},ua:ua,getFlashPlayerVersion:function(){return{major:ua.pv[0],minor:ua.pv[1],release:ua.pv[2]}},hasFlashPlayerVersion:hasPlayerVersion,createSWF:function(attObj,parObj,replaceElemIdStr){if(ua.w3)return createSWF(attObj,parObj,replaceElemIdStr);else return undefined},showExpressInstall:function(att,par,replaceElemIdStr,
callbackFn){if(ua.w3&&canExpressInstall())showExpressInstall(att,par,replaceElemIdStr,callbackFn)},removeSWF:function(objElemIdStr){if(ua.w3)removeSWF(objElemIdStr)},createCSS:function(selStr,declStr,mediaStr,newStyleBoolean){if(ua.w3)createCSS(selStr,declStr,mediaStr,newStyleBoolean)},addDomLoadEvent:addDomLoadEvent,addLoadEvent:addLoadEvent,getQueryParamValue:function(param){var q=doc.location.search||doc.location.hash;if(q){if(/\?/.test(q))q=q.split("?")[1];if(param==null)return urlEncodeIfNecessary(q);
var pairs=q.split("&");for(var i=0;i<pairs.length;i++)if(pairs[i].substring(0,pairs[i].indexOf("="))==param)return urlEncodeIfNecessary(pairs[i].substring(pairs[i].indexOf("=")+1))}return""},expressInstallCallback:function(){if(isExpressInstallActive){var obj=getElementById(EXPRESS_INSTALL_ID);if(obj&&storedAltContent){obj.parentNode.replaceChild(storedAltContent,obj);if(storedAltContentId){setVisibility(storedAltContentId,true);if(ua.ie&&ua.win)storedAltContent.style.display="block"}if(storedCallbackFn)storedCallbackFn(storedCallbackObj)}isExpressInstallActive=
false}}}}();function FABridge(target,bridgeName){this.target=target;this.remoteTypeCache={};this.remoteInstanceCache={};this.remoteFunctionCache={};this.localFunctionCache={};this.bridgeID=FABridge.nextBridgeID++;this.name=bridgeName;this.nextLocalFuncID=0;FABridge.instances[this.name]=this;FABridge.idMap[this.bridgeID]=this;return this}FABridge.TYPE_ASINSTANCE=1;FABridge.TYPE_ASFUNCTION=2;FABridge.TYPE_JSFUNCTION=3;FABridge.TYPE_ANONYMOUS=4;FABridge.initCallbacks={};
...
"x-user-defined");if(swfobject.ua.ieVersion>10)request.responseType="arraybuffer";return request};HttpAjaxPartRetriever.prototype.convertResponseBodyToText=function(binary){return IEBinaryToArray_Arr(binary).replace(/[\s\S]/g,function(t){var v=t.charCodeAt(0);return String.fromCharCode(v&255,v>>8)})+IEBinaryToArray_Arrl(binary)};
HttpAjaxPartRetriever.prototype.getRequestData=function(request,start){start=start||0;var responseString="",size;var data=null;if(swfobject.ua.ie)data=request.responseBody;else if(swfobject.ua.ieVersion>10)data=request.response;else data=request.responseText;if(swfobject.ua.ie){var responseArray=this.convertResponseBodyToText(data);size=responseArray.length;var arr=new Array(size);for(var j=0;j<size;j++)arr[j]=responseArray.charCodeAt(j)&255;var chunks=size/1024+1;for(var i=0;i<chunks;i++){var index=
i*1024;responseString+=String.fromCharCode.apply(null,arr.slice(index,index+1024))}}else if(null!==swfobject.ua.ieVersion&&swfobject.ua.ieVersion>10){var byteArray=new Uint8Array(data);var n=byteArray.length;var j=start;while(j<n){arr=byteArray.subarray(j,j+1024);j+=1024;responseString+=String.fromCharCode.apply(null,arr)}}else{var arr=new Array(1024);var j=start;var result="";while(j<data.length){for(var k=0;k<1024&&j<data.length;++k){arr[k]=data.charCodeAt(j)&255;++j}responseString+=String.fromCharCode.apply(null,
arr.slice(0,k))}}return responseString};HttpAjaxPartRetriever.ReadyStatus={UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4};HttpAjaxPartRetriever.HttpStatus={OK:200,PARTIAL_CONTENT:206};
if(/msie/i.test(navigator.userAgent)&&!/opera/i.test(navigator.userAgent)||null!==swfobject.ua.ieVersion&&swfobject.ua.ieVersion<11){var script=document.createElement("script");script.type="text/vbscript";var vbscriptData="Function IEBinaryToArray_Arr(Binary)\r\n"+"   IEBinaryToArray_Arr = CStr(Binary)\r\n"+"End Function\r\n"+"Function IEBinaryToArray_Arrl(Binary)\r\n"+'   IEBinaryToArray_Arrl = ""\r\n'+"   lastIndex = LenB(Binary)\r\n"+"   if LenB(Binary) mod 2 Then IEBinaryToArray_Arrl = ChrW(AscB(RightB(Binary,1)))\r\n"+
"End Function\r\n";script.text=vbscriptData;document.getElementsByTagName("head")[0].appendChild(script)}var _web_viewer_swfs={};function HttpAjaxPartRetrieverRequestRange(fid,rid,url,start,end,part){_web_viewer_swfs[fid][url].requestRange(rid,url,start,end,part)}function HttpAjaxPartRetrieverRequestOffset(fid,rid,url,offset,part){_web_viewer_swfs[fid][url].requestOffset(rid,url,offset,part)}
