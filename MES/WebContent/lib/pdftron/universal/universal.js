...
window.WebViewerUniversalView = Backbone.View.extend({
    ...
    resize: function() {
        ...
        if(this.model.get('type') === 'html5Mobile'){
            $('.hidden-mobile').hide();
            $('.visible-mobile').hide();
        }
    ...
    initializeGUI: function() {
        ...
        i18n.init(getI18nOptions(), function() {
            $('body').i18n();
            me.initializeJQueryUI();
            //do no initialize the toolbar until i18n is ready;
            me.initializeToolbar();

            me.model.set('i18nready', true);
        });
    },
    initializeJQueryUI: function(){
        $("#fitModes").buttonset();
        $("#tools").buttonset();
        ...
        $('#sidePanelToggle').button({
            ...
            var isOpen = myWebViewer.getShowSideWindow();
            if(isOpen){
                $(this).find('.ui-icon')
                    .addClass('ui-icon-custom-side-contact')
                    .removeClass('ui-icon-custom-side-expand');
            }else{
                $(this).find('.ui-icon')
                    .removeClass('ui-icon-custom-side-contact')
                    .addClass('ui-icon-custom-side-expand');
            }
        });
    },
    /**
    ...
    initializeToolbar: function() {
        ...
        $(window).add(iframeWindow).keydown(function(e) {
            ...
            // currentTarget should be the outer window!
            if (iframeWindow && e.currentTarget === window) {
        ...
        $("#pan").click(function() {
            myWebViewer.setToolMode(PDFTron.WebViewer.ToolMode.PanAndAnnotationEdit);
        });
        ...
        $("#layoutModeMenuButton").click(function() {
...
//                    }, 0);
                    ...
                    if (docElm.requestFullscreen) {
                        ...
                        $("#documentContainer iframe").width($("#documentContainer iframe").width() - 1);
        ...
        $('#sidePanelToggle').click(function() {
            var closed = myWebViewer.getShowSideWindow();
            myWebViewer.setShowSideWindow(!closed);
            $('#sidePanelToggle').change(); //trigger state change.
        });
    },
    /**
    ...
    initializeWebViewer: function() {
        ...
        if (this.model.get('documentUrl')) {
            options.initialDoc = this.model.get('documentUrl');
        }
        ...
        $(viewerElement).on('documentLoaded', {me: this, model: this.model, view: this.view}, function(event) {
            ...
            me.model.set('toolMode', PDFTron.WebViewer.ToolMode.PanAndAnnotationEdit);
        });
    ...
    initializeViewerSpecific: function() {
        if (myWebViewer.getViewerType() !== "html5Mobile") {
             $('.toolbar').show();
        }
    ...
    initializeDocument: function(event) {
        ...
        $("#toggleSidePanel").change(); //updates ui state
    },
    /**
    ...
    updateFitMode: function() {
        ...
        if (fitModeVal === PDFTron.WebViewer.FitMode.FitWidth) {
            $("#fitModes #fitWidth").prop('checked', true).trigger("change");
        } else if (fitModeVal === PDFTron.WebViewer.FitMode.FitPage) {
            $("#fitModes #fitPage").prop('checked', true).trigger("change");
        } else if (fitModeVal === PDFTron.WebViewer.FitMode.Zoom) {
            $("#fitModes input").prop('checked', false).trigger("change");
        }
    ...
    updateToolMode: function() {
        ...
        if (toolModeVal === PDFTron.WebViewer.ToolMode.PanAndAnnotationEdit || toolModeVal === PDFTron.WebViewer.ToolMode.Pan ) {
            $("#tools input[data-tool-mode=Pan]").prop("checked", true).trigger("change");
        } else if (toolModeVal === PDFTron.WebViewer.ToolMode.TextSelect) {
            $("#tools input[data-tool-mode=TextSelect]").prop("checked", true).trigger("change");
        } else {
            $("#tools input").prop("checked", false).trigger("change");
        }
