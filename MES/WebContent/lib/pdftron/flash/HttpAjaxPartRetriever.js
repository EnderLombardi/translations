...
HttpAjaxPartRetriever.prototype.createRequest = function(range, useCache) 
{
    ...
    if(swfobject.ua.ieVersion > 10) {
        // see comments below on why we do this now
...
HttpAjaxPartRetriever.prototype.getRequestData = function(request, start)
{
    ...
    } else if(swfobject.ua.ieVersion > 10) {
	...
	}  else if(null !== swfobject.ua.ieVersion && swfobject.ua.ieVersion > 10) {
        // IE11 String.charCodeAt still behaves differently then other browsers, however,
        // IE11 now disables VBScripts, so we need a third way to get the binary data...
        var byteArray = new Uint8Array(data);
        var n = byteArray.length;
        var j = start;
        while (j < n) {
            arr = byteArray.subarray(j, j + 1024);
            j += 1024;
            responseString += String.fromCharCode.apply(null, arr);
        }
    } else 
	{
...
if((/msie/i.test(navigator.userAgent) && !/opera/i.test(navigator.userAgent)) || 
    (null !== swfobject.ua.ieVersion && swfobject.ua.ieVersion < 11)) {
