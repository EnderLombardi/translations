...
$(function() {
    ...
    var $tabs = $('#tabs');
    var $annotationPanel = $('.annotationPanel');
    ...
    $(document).on("documentLoaded", function() {
        ...
        am.on('annotationChanged', function(e, annotations, action) {
            ...
            for (var j = 0; j < annotations.length; ++j) {
                ...
                if (action === "add") {
                    $li = readerControl.createAnnotationListItem(annotation);

                    var isInserted = false;
                    var lastElement = null;
                    var allAnnots = $annotationList.children('li');
                    // insert into the annotation list in page order
                    allAnnots.each(function() {
                        var $this = $(this);
                        var pageNumber = $this.data('annot').PageNumber;
                    ...
                    var replies = annotation.getReplies();

                    for (var i = 0; i < liList.length; i++) {
                        $li = $(liList[i]);
                        var annot = $li.data("annot");

                        // if we're a reply to this annotation then show it since it was hidden
                        var isReplyToAnnot = replies.indexOf(annot) > -1;
                        if (isReplyToAnnot) {
                            $li.show();
                        }
        ...
        am.on('annotationSelected', function(e, annotations, action) {
            ...
            if (action === "selected") {
                ...
                if (annotations.length === 1) {
                    ...
                    // only show the edit properties if they can't be modified
                    if (am.CanModify(annotation)) {
                        ...
                        if (annotation.StrokeThickness !== null && typeof annotation.StrokeThickness !== "undefined") {
                            ...
                            if(annotation instanceof Annotations.FreeTextAnnotation && annotation.Intent === Annotations.FreeTextAnnotation.Intent.FreeText){
                                $thicknessSlider.slider('option', 'min', 0);
                            }else{
                                $thicknessSlider.slider('option', 'min', 1);
                            }
                        } else {
                ...
                var elementsToSelect = $annotationList.find('li').filter(function() {
                    return annotations.indexOf($(this).data('annot')) !== -1;
                });
                elementsToSelect.addClass('ui-state-active');

                //scrollIntoView messes up the iframe's overflow, use this instead
                if ($(elementsToSelect).offset().top > $('#annotationListScroll').height() - $(elementsToSelect).height()
                        || $(elementsToSelect).offset().top < $('#annotationListScroll').scrollTop()) {
                    //scroll into view if it is not visible
                    $('#annotationListScroll').scrollTop($(elementsToSelect).offset().top); //scroll into view
                }
    ...
    function setupToolPanel() {
        ...
        $annotationList.selectable({
            stop: function() {
                ...
                docViewer.SetToolMode(Tools.AnnotationEditTool);
    ...
    function resizeAnnotationList() {
        $tabs.tabs("updateHeight", $annotationPanel.parent());
    }

    function resizeAnnotationListOld() {
    ...
    window.readerControl.createAnnotationListItem = function(annotation) {
        ...
        message += "<div class=\"datecreated\">" + annotation.DateCreated.toLocaleString() + "</div>";
