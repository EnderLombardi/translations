            ...
            var oColumn = new sap.ui.table.Column(
                    {
                        ...
                        showFilterMenuEntry : false,
                        width : "3rem",
//                        visible: airbus.mes.stationtracker.util.AssignmentManager.polypolyAffectation,
...
//        var iWidth = 47;
        ...
        var bFound = false;
        do{
            if(iHeight-iLines*(iWidth+1)< iFloor){
                bFound = true
            }else{
                iLines = iLines + iStep;
            }
...
//        var oNumberRows = Math.floor(oDim.height*0.0267 - 10.035);    //FIXME : Change if Rows' height CSS is modified
        ...
        oNumberRows = Math.max(this.findLinesNumber(e[a+'Height']),1);
...
//                airbus.mes.polypoly.oView.byId("oTablePolypoly").setFixedRowCount(3);
    ...
    setPolyFilters : function() {
        ...
        if (airbus.mes.polypoly.oView.byId("oTablePolypoly").getBinding("rows")) {
            ...
            var filterUA = new sap.ui.model.Filter("type", "Contains", "UA_");
            var aFilters = [filterLabels, filterUA];
            if ( airbus.mes.stationtracker.util.AssignmentManager.polypolyAffectation ) {
                airbus.mes.polypoly.oView.byId("oTablePolypoly").getBinding("rows").filter(aFilters);
            }else{
                airbus.mes.polypoly.oView.byId("oTablePolypoly").getBinding("rows").filter()
            }
        ...
        //test
        //console.log("==2== :" + sap.ui.getCore().byId("typeRow"))
    },
    ...
    initiatePolypoly : function(){
        ...
        var aControlsId = ["AddPolypoly", "navBack", "kpiHeaderToolbar"];
        this.setModePoly();
        ...
        //test
        //console.log("Final :" + sap.ui.getCore().byId("typeRow"))
    },
...
//        airbus.mes.polypoly.PolypolyManager.getValueSelected = sap.ui.getCore().byId("polypoly--polypolySearchField").getValue();
        ...
        if (oBindingInfo != undefined) {
            ...
            aQASelList
            .forEach(function(el) {
                for (var i = 0; i < sap.ui.getCore().getModel("columnModel").oData.qa.length; i++) {
                    if (sap.ui.getCore().getModel("columnModel").oData.qa[i].CERTIFICATION == el.label) {
                        sap.ui.getCore().getModel("columnModel").oData.qa[i].selected = true;
                    } else if (sap.ui.getCore().getModel("columnModel").oData.qa[i].selected == undefined) {
                        sap.ui.getCore().getModel("columnModel").oData.qa[i].selected = false;
                    }
                }
            });
        } else {
            ...
            sap.ui.getCore().getModel("columnModel").oData.qa.forEach(function(el) {
                el.selected = false;
            })
        }
        that.colDialog.setModel(sap.ui.getCore().getModel("columnModel"), "columnModel");
    ...
    insertColumnPopup: function() {
        ...
        var sName = sap.ui.getCore().getModel("columnModel").oData.competency;
        var sNeed3 = sap.ui.getCore().getModel("columnModel").oData.need3;
        var sNeed4 = sap.ui.getCore().getModel("columnModel").oData.need4;
        var aQAfiltered = sap.ui.getCore().getModel("columnModel").oData.qa.filter(
                function(el) {
                    return el.selected;
                }).map(function(el) {
    ...
    openConfirmDelete : function(oEvt) {
        ...
        sap.ui.getCore().getModel("columnModel").setData({techname : oBindingInfo.getProperty("techname")});
    ...
    onConfirmDelete : function() {
        airbus.mes.polypoly.PolypolyManager.deleteColumn(sap.ui.getCore().getModel("columnModel").getData().techname);
    },
...
//    },
    ...
    onIconAffectClick : function(oEvt){
        ...
        airbus.mes.stationtracker.util.AssignmentManager.polypolyAssignment.selectedUser = airbus.mes.polypoly.PolypolyManager.polypolyIndex;
        ...
        airbus.mes.stationtracker.util.AssignmentManager.handleLineAssignment("S", false);
