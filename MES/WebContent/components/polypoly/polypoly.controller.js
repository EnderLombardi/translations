...
//						var columnModel = new sap.ui.model.json.JSONModel();
					...
					onBeforeRendering : function() {
						// var
						// a=airbus.mes.polypoly.oView.byId("headerPolypoly");
						// if(airbus.mes.polypoly.PolypolyManager.isPolypolyEditable()
						// && airbus.mes.polypoly.oView.getContent().indexOf(a)
						// < 0){
					...
					// var oData =
					// sap.ui.getCore().getModel("mii").getData().Rowsets;
					// if (oData.Rowset && oData.Rowset.length > 0 &&
					// oData.Rowset[0].Row) {
					// var oMiiData =
					// sap.ui.getCore().getModel("mii").getData();
					// var oTableData =
					// airbus.mes.polypoly.PolypolyManager.createTableData(oMiiData);
					// var mTableModel = new
					// sap.ui.model.json.JSONModel(oTableData);
					// airbus.mes.polypoly.PolypolyManager.internalContext.oModel
					// = mTableModel;
					...
					openQAPopup : function() {
						...
						if (!airbus.mes.polypoly.QADialog) {
							airbus.mes.polypoly.QADialog = sap.ui.xmlfragment(
									"airbus.mes.polypoly.QAPopup", that);
						}
						airbus.mes.polypoly.QADialog
								.setModel(airbus.mes.polypoly.PolypolyManager.internalContext.oModelQA);
						airbus.mes.polypoly.QADialog.setModel(
								airbus.mes.polypoly.oView
										.getModel("PolypolyI18n"),
								"PolypolyI18n");
						sap.ui
								.getCore()
								.byId("oTableQA")
								.setVisibleRowCount(
										airbus.mes.polypoly.PolypolyManager.internalContext.iLinesQA);
					...
					createColumn : function(sId, oContext) {
															...
															new sap.m.Image(
																	{
																		...
																		src : {
																			...
																			formatter : function(
																					oRow) {
																				if (oRow != null) {
																					return oRow.picture ? airbus.mes.shell.UserImageManager
																							.getUserImage(
																									this
																											.getId(),
																									oRow.picture)
																		...
																		visible : {
																			...
																			formatter : function(
																					type) {
																				return type == "UA_A"
																						|| (type == "UA_P" && airbus.mes.settings.AppConfManager
																								.getConfiguration("MES_PHOTO_DISPLAY"));
																			}
																		}
																	}),
															new sap.m.Text(
																		...
																		visible : {
																			...
																			formatter : function(
																				...
																				return airbus.mes.settings.AppConfManager
																						.getConfiguration("MES_PHOTO_NAME");
																			}
																		}
																	}) ]
												})
						...
						case "selected":
							var oColumn = new sap.ui.table.Column({
								...
								width : "3rem",
								// visible:
								// airbus.mes.stationtracker.AssignmentManager.polypolyAffectation,
								visible : false, // This column may no longer
													// be used
															...
															new sap.ui.core.Icon(
																	{
																		...
																		tooltip : {
																			...
																			formatter : function(
																					icon) {
																				if (airbus.mes.polypoly.util.Formatter
																						.isVisible()) {
																					return icon;
																				} else {
																					return "Affect";
						...
						case "column":
							...
							// if
							// (airbus.mes.polypoly.PolypolyManager.globalContext.tabSelected
							// == "polypoly") {
							// // if (true){
															...
															new sap.m.Select(
																	{
																		...
																		enabled : {
																			...
																			formatter : function(
																					type) {
																				// return
																				// formatter.isPolypolyEditable();
																				return airbus.mes.polypoly.util.Formatter
																						.isVisible();
																			},
																		},
																	}),
					...
					onSelectLevelChange : function(oEvt) {
						...
						airbus.mes.polypoly.PolypolyManager.updateColumn(sName,
								sQA, sNeed3, sNeed4, sTechname);
					},
					...
					setRowCountVisible : function(bAssign) {
						...
						var oNumberRows = Math
								.floor(oDim.height * 0.0267 - 10.035); // FIXME
																		// :
																		// Change
																		// if
																		// Rows'
						...
						if (bAssign) {
							var aListRows = airbus.mes.polypoly.oView.byId(
									"oTablePolypoly").getBinding("rows").oList;
							aListRows = aListRows.filter(function(el) {
								return el.type == "UA_P" || el.type == "UA_A"
							});
							oNumberRows = Math.min(oNumberRows,
									aListRows.length + 1);
						}
						airbus.mes.polypoly.oView.byId("oTablePolypoly")
								.setVisibleRowCount(oNumberRows);
					},

					filterUA : function() {
						if (sap.ui.getCore().byId("typeRow") /*
																 * &&
																 * sap.ui.getCore().byId("FilterClockedUsers")
																 */) {
							var filterLabels = new sap.ui.model.Filter("type",
									"EQ", "LABEL_RP");
							var filterUA = new sap.ui.model.Filter("type",
									"Contains", "UA_");
							var aFilters = [ filterLabels, filterUA ];
							// if(sap.ui.getCore().byId("FilterClockedUsers").getSelected())
							// filterUA = new sap.ui.model.Filter("type", "EQ",
							// "UA_CI","UA_NCD");
							// else
							...
							if (nav.getCurrentPage().getId() != "polypolyPage") {
								...
								// //FIXME : Depends on PopUp size small = 8
								airbus.mes.polypoly.oView.getController()
										.setRowCountVisible(true);
					...
					hideContent : function(aControlsId) {
						aControlsId.forEach(function(el) {
							var sControlID = el;
							var bVisible = airbus.mes.polypoly.PolypolyManager
									.isPolypolyEditable();
							airbus.mes.polypoly.oView.byId(sControlID)
									.setVisible(bVisible);
						})
						airbus.mes.polypoly.oView.rerender();
					},
					...
					onRPSuggest : function(oEvt) {
						...
						airbus.mes.polypoly.oView.byId("polypolySearchField")
								.getBinding("suggestionItems").filter(filters);
						airbus.mes.polypoly.oView.byId("polypolySearchField")
								.suggest();
					...
					clearFilters : function() {
						...
						if (airbus.mes.polypoly.oView.byId("oTablePolypoly")
								.getBinding("rows")) {
							var filterLabels = new sap.ui.model.Filter("type",
									"EQ", "LABEL_RP");
							var filterUA = new sap.ui.model.Filter("type",
							...
							airbus.mes.polypoly.oView.byId(
					...
					openColumnPopup : function(oEvt) {
						...
						if (oBindingInfo != undefined) {
							...
							that.colDialog.setTitle(airbus.mes.polypoly.oView
									.getController().getI18n("EditPolypoly"));
							...
							columnModel
									.setData({
										...
										techname : oBindingInfo
												.getProperty("techname"),
										qa : aQAList,
										need3 : airbus.mes.polypoly.PolypolyManager.internalContext.oModel.oData.rows[1][oBindingInfo
												.getProperty("techname")],
										need4 : airbus.mes.polypoly.PolypolyManager.internalContext.oModel.oData.rows[2][oBindingInfo
												.getProperty("techname")]
									});
							aQASelList
									.forEach(function(el) {
							...
							that.colDialog.setTitle(airbus.mes.polypoly.oView
									.getController().getI18n("CreatePolypoly"));
							columnModel.setData({
								competency : "",
								techname : "",
								qa : aQAList,
						...
						that.colDialog.setModel(airbus.mes.polypoly.oView
								.getModel("PolypolyI18n"), "PolypolyI18n");
						that.colDialog.open();
						...
						if (aQAfiltered.length <= 5 && sTechname != ""
							...
							if (airbus.mes.polypoly.PolypolyManager.internalContext.saveContext == "CREATE") {
								airbus.mes.polypoly.PolypolyManager
										.createColumn(sName, sQA, sNeed3,
												sNeed4, sTechname);
							} else {
								airbus.mes.polypoly.PolypolyManager
										.updateColumn(sName, sQA, sNeed3,
												sNeed4, sTechname);
							}
						} else if (sTechname == "") {
							sap.m.MessageToast.show(airbus.mes.polypoly.oView
									.getController().getI18n("TechNameEmpty"));
						} else if (sName == "") {
							sap.m.MessageToast.show(airbus.mes.polypoly.oView
									.getController().getI18n("DescEmpty"));
						} else if (aQAfiltered.length > 5) {
							sap.m.MessageToast.show(airbus.mes.polypoly.oView
									.getController().getI18n("TooManyQA"));
						}
					},
					...
					openConfirmDelete : function(oEvt) {
						...
						if (!airbus.mes.polypoly.delDialog) {
							airbus.mes.polypoly.delDialog = sap.ui.xmlfragment(
									"airbus.mes.polypoly.confirmDelete",
									airbus.mes.polypoly.oView.getController());
						}
						airbus.mes.polypoly.delDialog.setModel(
								airbus.mes.polypoly.oView
										.getModel("PolypolyI18n"),
								"PolypolyI18n");
						...
						if (newPos != oldPos) {
							if ((oEvt.getParameters().newPos - 6) < 0
									...
									|| airbus.mes.polypoly.util.Formatter
											.isVisible() == false) {
								sap.m.MessageToast
										.show(airbus.mes.polypoly.oView
												.getController().getI18n(
														"Unauthorized"));
								...
								airbus.mes.polypoly.PolypolyManager.moveColumn(
										sTechname, newPos);
							}
						}
					},
					...
					// return
					// airbus.mes.polypoly.PolypolyManager.globalContext.bEditable;
					// } else {
					// return false;
					// }
					...
					getI18n : function(str) {
						return airbus.mes.polypoly.oView.getModel(
								"PolypolyI18n").getProperty(str)
					},
					...
					onIconAffectClick : function(oEvt) {
						...
						if (!airbus.mes.polypoly.PolypolyManager.oViewController) {
							airbus.mes.polypoly.PolypolyManager.oViewController = airbus.mes.polypoly.oView
									.getController();
						}
						airbus.mes.polypoly.PolypolyManager.oViewController.oUserIcon = oEvt
								.getSource();
						airbus.mes.polypoly.PolypolyManager.polypolyIndex = airbus.mes.polypoly.PolypolyManager.oViewController.oUserIcon
								.getModel()
								.getProperty(
										airbus.mes.polypoly.PolypolyManager.oViewController.oUserIcon
						...
						if (sERP_ID == "---" || sERP_ID == " ") {
							ModelManager.messageShow(airbus.mes.polypoly.oView
									.getController().getI18n("InvalidID")
									+ sERP_ID);
							return false;
						}

						airbus.mes.stationtracker.AssignmentManager
								.handleLineAssignment("S", false);

						// if
						// (!airbus.mes.polypoly.PolypolyManager.oViewController.oDialogConfirmationPoyPoly)
						// {
						// airbus.mes.polypoly.PolypolyManager.oViewController.oDialogConfirmationPoyPoly
						// =
						// sap.ui.xmlfragment("airbus.mes.polypoly.userAffectationConfirmationPolyPoly",airbus.mes.polypoly.oView.getController());
						// }
					...
					// = oEvt.getSource();
					// airbus.mes.polypoly.PolypolyManager.polypolyIndex =
					// airbus.mes.polypoly.PolypolyManager.oViewController.checkBox.getModel().getProperty(airbus.mes.polypoly.PolypolyManager.oViewController.checkBox.getBindingContext().getPath());
					// if
					...
					// }
					// if
					// (airbus.mes.polypoly.PolypolyManager.oViewController.checkBox.getSelected())
					// {
					// if
					// (sap.ui.getCore().byId("toogleAffectConfirm").getState()
					// === true) {
					// if
					// (!airbus.mes.polypoly.PolypolyManager.oViewController.oDialogConfirmationPoyPoly)
					// {
					// airbus.mes.polypoly.PolypolyManager.oViewController.oDialogConfirmationPoyPoly
					// =
					// sap.ui.xmlfragment("airbus.userAffectationConfirmationPolyPoly",airbus.mes.polypoly.PolypolyManager.oViewController);
					// }
					// var erp_id =
					// airbus.mes.polypoly.PolypolyManager.polypolyIndex.ERP_IDModelManager.chkUserOprCertificatePolyPoly(erp_id,airbus.mes.polypoly.PolypolyManager.oViewController.checkBox);
					// return;
					// }
					...
					// }
					// if
					// ((ModelManager.polypoly_UserSave.some(function(element) {
					// return (element.ERP_ID ==
					// airbus.mes.polypoly.PolypolyManager.polypolyIndex.ERP_ID);
					// })) === false) {
					// ModelManager.polypoly_UserSave.push(airbus.mes.polypoly.PolypolyManager.polypolyIndex);
					// }
