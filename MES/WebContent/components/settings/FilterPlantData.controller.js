sap.ui.controller("airbus.mes.settings.FilterPlantData", {
	...
	onSelectionChange : function(oEvt) {
		...
		this.findElement(this.selectTree, oEvt.getSource().getId().split("--")[1]).childs.forEach(function(oElement) {
	...
	/**
	 * Fire when the user select msn and save takt value
	 */
	msnSelected : function() {
		
		var sPath = airbus.mes.settings.oView.byId("ComboBoxMSN").getSelectedItem().getBindingContext("plantModel").sPath;
		var oModel = airbus.mes.settings.oView.getModel("plantModel").getProperty(sPath);
		
		airbus.mes.settings.ModelManager.taktStart = oModel.begin;
		airbus.mes.settings.ModelManager.taktEnd = oModel.end;
		airbus.mes.settings.ModelManager.taktDuration = oModel.duration;
