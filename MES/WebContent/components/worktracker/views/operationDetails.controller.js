sap.ui.controller("airbus.mes.worktracker.views.operationDetails", {
...
*/
	onInit: function() {
		...
		this.getView().setModel(new sap.ui.model.json.JSONModel(),"tasklist");	 
		this.getView().getModel("tasklist").loadData("local/tasklist.json",null,false);
	...
	onCreateDisruption:function(){
		...
		this.getView().byId("disruptionsHeader").setVisible(false);
	},
	onCancelCreateDisruption:function(){
		this.getView().byId("disruptionsHeader").setVisible(true);
...
*/
	...
	expandOperationDetailPanel: function(oEvent){
		this.getView().byId("opDetailExpandButton").toggleStyleClass("invisible");
		this.getView().byId("operationDetailPanel").setExpanded();
	},
	...
	setProgressScreenBtn: function(progressBtnStatus,actionBtnStatus,activateBtnStatus){
		this.getView().byId("btnAdd").setEnabled(progressBtnStatus);
		this.getView().byId("btnReduce").setEnabled(progressBtnStatus);
		this.getView().byId("btnPause").setVisible(actionBtnStatus);
		this.getView().byId("btnConfirm").setVisible(actionBtnStatus);
		this.getView().byId("btnComplete").setVisible(actionBtnStatus);
		this.getView().byId("btnActivate").setVisible(activateBtnStatus);
	},
	...
	activateOperation : function()
	{
		...
		this.getView().byId("operationStatus").setText("In Progress");
	},
	pauseOperation : function()
	{
		...
		this.getView().byId("operationStatus").setText("Paused");
	},	
	confirmOperation : function()
	{
		  if (!this._oPartialConfirmDialog) {  
          
             this._oPartialConfirmDialog = sap.ui.xmlfragment("airbus.mes.worktracker.fragments.operationConfirmDialog", this);  
           
             this.getView().addDependent(this._oPartialConfirmDialog);  
          }  
		  this._oPartialConfirmDialog.open();
		/*var iProgress = this.getView().byId("progressSlider").getValue();
	...
	completeOperation : function()
	{
		if (!this._oFullConfirmDialog) {  
            ...
            this._oFullConfirmDialog = sap.ui.xmlfragment("airbus.mes.worktracker.fragments.operationCompleteDialog", this);  
          
            this.getView().addDependent(this._oFullConfirmDialog);  
         }  
	...
	onFullConfirmation : function()
	{
		...
		this.getView().byId("progressSlider").setEnabled(false);
		this.getView().byId("operationStatus").setText("Completed");
