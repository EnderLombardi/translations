	...
	.controller(
		"airbus.mes.disruptions.ViewDisruption",
		{
			...
			turnOnOffButtons : function() {
				...
				// if operation is not complete complete, we can create a
				// disruption
			...
			/*******************************************************************
			 * Filter comments by removing comments not related to the selected
			 * disruption
			 */
			applyFiltersOnComments : function() {
				var listItems = this.getView().byId("disrptlist").getItems();
				$.each(listItems, function(key, oItem) {
					/** Apply filters on Message Comments * */

					// Get Message Ref from current list
					var messageRef = oItem.getBindingContext("operationDisruptionsModel").getObject().MessageRef;

					// Get Binding of Comment list for Current item
					var oBinding = oItem.getContent()[0].getContent()[3].getBinding("items");
			...
			onCloseDisruption : function(oEvt) {
				...
				var commentBox = this.getView().byId(this.getView().sId + "--commentBox-" + this.getView().sId + "--disrptlist-" + listnum);
				commentBox.setVisible(false);

				var submitCommentId = sap.ui.getCore().byId(this.getView().sId + "--addComment-" + this.getView().sId + "--disrptlist-" + listnum);
				...
				// give the id of add comment button to hide it if operation
				// closed
			...
			onOpenDisruptionComment : function(title, msgRef, sPath, okEvent, sStatus) {
				...
				sap.ui.getCore().byId("disruptionCommentOK").detachPress(this.pressEvent);
			...
			onDeleteDisruption : function(oEvt) {
				...
				if (status == airbus.mes.disruptions.Formatter.status.acknowledged) {
					sap.m.MessageBox.error(airbus.mes.disruptions.oView.viewDisruption.getModel("i18nModel").getProperty("disruptionDeleteError"));
				} else {
			...
			onRejectDisruption : function(oEvt) {
				...
				var title = airbus.mes.disruptions.oView.viewDisruption.getModel("i18nModel").getProperty("rejectDisruption");
				var msgRef = oEvt.getSource().getBindingContext("operationDisruptionsModel").getObject("MessageRef");
				var sPath = oEvt.getSource().getBindingContext("operationDisruptionsModel").sPath;
			...
			onConfirmRejection : function(oEvent) {
				...
				var isSuccess = airbus.mes.disruptions.ModelManager.rejectDisruption(comment, msgRef, sStatus, sMessage, i18nModel);
			...
			onRefuseDisruption : function(oEvt) {
				...
				var title = airbus.mes.disruptions.oView.viewDisruption.getModel("i18nModel").getProperty("refuseDisruption");
				var msgRef = oEvt.getSource().getBindingContext("operationDisruptionsModel").getObject("MessageRef");
				var sPath = oEvt.getSource().getBindingContext("operationDisruptionsModel").sPath;
			...
			onConfirmRefuse : function(oEvent) {
				...
				// Call Disruption Service
				var isSuccess = airbus.mes.disruptions.ModelManager.refuseDisruption(comment, msgRef, sMessage, i18nModel);
			...
			hideCommentBox : function(oEvt) {
				...
				var commentBoxId = this.getView().byId(this.getView().sId + "--commentBox-" + this.getView().sId + "--disrptlist-" + listnum);
			...
			onAckDisruption : function(oEvt) {
				...
				sap.ui.getCore().byId("disruptionAckSpathMsgRef").setText(msgRef);
			...
			onMarkSolvedDisruptionComment : function() {
				...
				var isSuccess = airbus.mes.disruptions.ModelManager.markSolvedDisruption(msgRef, comment, i18nModel);
				...
				if (isSuccess) {
					...
					sap.ui.getCore().byId("ViewDisruptionView").getModel("operationDisruptionsModel").getProperty("/Rowsets/Rowset/1/Row").push(oComment);

					sap.ui.getCore().byId("ViewDisruptionView").getModel("operationDisruptionsModel").refresh();
				}
			},
			 ...
			 * 
			 * if (!oevent.oSource.getExpanded()) return;
			 * 
			 * this.expandedDisruptionPanel = oevent.getSource().getId();
			 * 
			 * var disruptions = this.getView().byId("disrptlist");
			 * $(disruptions.getItems()) .each( function() { var currentPanel =
			 * this .getContent()[0]; if (oevent.getSource().getId() !=
			 * currentPanel .getId()) currentPanel.setExpanded(false) });
			 *  },
			 */
			...
			onEscalateDisruption : function(oEvent) {
				...
				var msgRef = oEvent.getSource().getBindingContext("operationDisruptionsModel").getObject("MessageRef");

				var i18nModel = airbus.mes.disruptions.oView.viewDisruption.getModel("i18nModel");

				// Call Escalate Service
				var isSuccess = airbus.mes.disruptions.ModelManager.escalateDisruption(msgRef, i18nModel);

				// Change Severity level in model
				if (isSuccess) {
					var sPath = oEvent.getSource().getBindingContext("operationDisruptionsModel").sPath;

					this.getView().getModel("operationDisruptionsModel").getProperty(sPath).EscalationLevel = parseInt(this.getView().getModel(
						"operationDisruptionsModel").getProperty(sPath).EscalationLevel, 10) + 1;
			...
			onReportDisruption : function(oEvent) {
				 ...
				 * SD-SP1604983-CDP-010 and SD-SP1604983-DDR-005 //To fill
				 * initially logged in user in issuer field var sUserId =
				 * sap.ui.getCore().getModel("userDetailModel").getProperty("/Rowsets/Rowset/0/Row/0/user_id")
				 * 
				 * var sUserName =
				 * (sap.ui.getCore().getModel("userDetailModel").getProperty("/Rowsets/Rowset/0/Row/0/last_name") +" "
				 * +sap.ui.getCore().getModel("userDetailModel").getProperty("/Rowsets/Rowset/0/Row/0/first_name"))
				 * 
				 * var oItem = new sap.ui.core.Item({key: sUserId ,text:
				 * sUserName});
				 * 
				 * 
				 * if
				 * (sap.ui.getCore().byId("createDisruptionView--selectIssuer").getSelectedKey()=="") {
				...
				// destroying Material List dialog which might have already
				// loaded and will show inconsistent data otherwise
