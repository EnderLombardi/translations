...
sap.ui
	.controller(
		"airbus.mes.factoryView.FactoryView",
		{
			...
			onClickStation5 : function(evt) {
				...
				case "Unload":
					if (evt.getSource().getParent().getItems()[0].getMsn() == "") {
						...
						airbus.mes.factoryView.util.ModelManager.messageShow("MSN already unloaded");
					} else {
			...
			openDialogUnload : function() {
				...
				sap.ui.commons.MessageBox.confirm('Do you want to unload the ' + this.stype + ' from MSN ' + MSN + ' ' + HAND + ' ?', fnCallbackConfirm,
					'Station 5 ' + this.stype);
			...
			onAfterRendering : function() {
				if (this.getView().byId("stationLine1").getBinding("items")) {
					var filters = [];
					...
					var oProductionLine1 = this.getView().byId("stationLine1").getBinding("items").filter(filters);

					filters.pop();
					filters.push(filterLine2);
					var oProductionLine2 = this.getView().byId("stationLine2").getBinding("items").filter(filters);

					filters.pop();
					filters.push(filterLine3);
					var oProductionLine3 = this.getView().byId("stationLine3").getBinding("items").filter(filters);

					filters.pop();
					filters.pop();
					filters.push(filterLine4);
					var oProductionLine4 = this.getView().byId("stationLine5").getBinding("items").filter(filters);
				}
			},
			pulse : function(oEvt) {
				...
				// Confirmation box to check if the station has been previously
				// loaded on station 05 or not
				...
				if (sap.ui.getCore().byId("idFactoryView--stationLine" + line).getItems()[6].getMsn() != ''
					&& sap.ui.getCore().byId("idFactoryView--stationLine" + line).getItems()[7].getStationHeadColor() != "green" && station_l1 != "RP") {
					sap.ui.commons.MessageBox
						.show(
							...
							sap.ui.commons.MessageBox.Icon.WARNING, 'Warning', [ sap.ui.commons.MessageBox.Action.YES, sap.ui.commons.MessageBox.Action.NO ],
							fnCallbackMessageBox, sap.ui.commons.MessageBox.Action.YES);
					function fnCallbackMessageBox(bResult) {
						if (bResult == "YES") {
							if (valueStationL1 == "ne") {
								airbus.mes.factoryView.util.ModelManager.pulseLine(line);
							} else {
								airbus.mes.factoryView.util.ModelManager.pulse(line, value_station_l1);
							}
						}
					}
			...
			/**
			 * Called when pulse drop down list is selected. It enables the the
			 * pulse button if a value from drop down is selected
			 * 
			 * @input oEvt {object}
			 */
